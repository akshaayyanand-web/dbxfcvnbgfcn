...
const CLASSIFICATION_LABEL = {
  unreviewed: 'Unreviewed', confirmed: 'Confirmed match', partial: 'Partial match',
  false: 'False match', no_match: 'No match',
};
...
const PRESETS = {
  ...
  sunak: {file: 'demo_payload_sunak.json', name: 'Rishi Sunak', entityType: 'any', label: 'Rishi Sunak — PEP example'},
...
async function loadPresentationDemo(presetKey) {
  ...
  syncOptional();
  $('#nationality').value = preset.nationality || '';
  $('#dob_year').value = preset.dobYear || '';
  $('#jurisdiction').value = preset.jurisdiction || '';
...
function renderEmptyState(payload) {
  ...
  if (media && media.manual_search) {
    banner.innerHTML += `<h3>Adverse media screening</h3>
      <div class="amber">
...
async function saveAdverseMediaReview(name, media) {
  ...
  try {
    const response = await fetch('/api/adverse_media/review', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
        name,
